...
from phishing_detection_model import __version__ as _version
...
def run_training() -> None:
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        data[config.model_config.features[0]],  # predictors
    ...
    class_ = phishing_detection_pipe.predict(X_train)
    pred = phishing_detection_pipe.predict_proba(X_train)[:, 1]
    ...
    class_ = phishing_detection_pipe.predict(X_test)
    pred = phishing_detection_pipe.predict_proba(X_test)[:, 1]
    ...
    save_pipeline(pipeline_to_persist=phishing_detection_pipe)
